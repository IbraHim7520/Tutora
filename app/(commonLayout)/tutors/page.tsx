...
import { Users, Search, SlidersHorizontal, BookOpen } from 'lucide-react';
...
const TutorPage = () => {
  ...
  useEffect(() => {
    const fetchTutors = async () => {
      try {
        setIsLoading(true);
        const response = await fetch(`${process.env.NEXT_PUBLIC_BACKKEND_URL}/api/v1/tutors/tutor/all-tutors`);
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-4 mb-10">
          ...
          <div className="flex items-center gap-3 w-full md:w-auto">
            <p className="text-sm text-gray-500 hidden md:block">
              Showing <span className="font-bold text-gray-900">{filteredTutors.length}</span> tutors
            </p>
        ...
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 bg-white rounded-3xl border-2 border-dashed border-gray-200 text-center">
            ...
            <button 
                onClick={() => {setSearchQuery(""); setFilteredTutors(allTutors);}}
